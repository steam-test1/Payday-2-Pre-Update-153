					...
					if type(job) == "table" then
						for _, job_id in ipairs(job) do
							...
							for _, difficulty in ipairs(achievement_data.difficulty) do
								if 0 < managers.statistics:completed_job(job_id, difficulty) then
						...
						for _, difficulty in ipairs(achievement_data.difficulty) do
							if 0 < managers.statistics:completed_job(job, difficulty) then
								table.remove(available_jobs, id)
							end
						end
					end
...
function MissionEndState:on_statistics_result(best_kills_peer_id, best_kills_score, best_special_kills_peer_id, best_special_kills_score, best_accuracy_peer_id, best_accuracy_score, most_downs_peer_id, most_downs_score, total_kills, total_specials_kills, total_head_shots, group_accuracy, group_downs)
	...
	local level_id, all_pass, total_kill_pass, total_accuracy_pass, total_headshots_pass, total_downed_pass, level_pass, levels_pass, num_players_pass, diff_pass, is_dropin_pass
