...
function CrimeNetContractGui:init(ws, fullscreen_ws, node)
	...
	local has_ghost_bonus = 0 < ghost_bonus_mul
...
function CrimeNetContractGui:_check_level_up(levels_gained)
	local gonna_level_up = 1 <= levels_gained
...
function CrimeNetContractGui:set_potential_rewards(show_max)
	...
	if alive(premium_text) then
		...
		if #start_ci ~= #end_ci then
			...
			for i = 1, #start_ci do
				premium_text:set_range_color(start_ci[i], end_ci[i], not (i ~= 1 or can_afford) and tweak_data.screen_colors.pro_color or tweak_data.screen_colors.button_stage_2)
			end
		end
	end
end
