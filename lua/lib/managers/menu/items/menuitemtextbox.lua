...
function MenuItemTextBox:enter_text(row_item, o, s)
	if not (row_item and alive(row_item.gui_text) and self._editing) or self:_ctrl() then
		return
	end
...
function MenuItemTextBox:handle_key(row_item, o, k)
	...
	elseif k == Idstring("up") then
		local line_breaks = text:line_breaks()
		if #line_breaks < 2 or 2 <= #line_breaks and e <= line_breaks[2] then
	...
	elseif k == Idstring("down") then
		local line_breaks = text:line_breaks()
		if #line_breaks < 2 or 2 <= #line_breaks and e > line_breaks[#line_breaks] then
			text:set_selection(n, n)
		else
			table.insert(line_breaks, n)
			line_breaks[1] = -1
