...
function ContractBoxGui:create_contract_box()
	...
	if contact_data then
		...
		local total_payout_text = total_payout_min < total_payout_max and managers.localization:text("menu_number_range", {min = payout_text_min, max = payout_text_max}) or payout_text_min
...
function ContractBoxGui:create_character_text(peer_id, x, y, text, icon)
	...
	if self._peers[peer_id]:visible() and self._peers[peer_id]:text() ~= "" then
		...
		self._peers_spree[peer_id]:set_visible(self._enabled and game_state_machine:gamemode().id == GamemodeCrimeSpree.id and 0 <= level)
