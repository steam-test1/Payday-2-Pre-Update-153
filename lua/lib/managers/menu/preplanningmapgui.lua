...
function PrePlanningMapGui:_update_breakdown()
	...
	if current_plans and 0 < #current_plans then
	...
	local category, ct
	if current_types and 0 < #current_types then
		t = {}
		...
		for category, types in pairs(ct) do
			...
			for _, type in ipairs(types) do
				count = t[type]
				text = breakdown_panel:text({
					text = (1 < count and tostring(count) .. "x " or "") .. managers.preplanning:get_type_name(type),
					font = font,
					font_size = font_size,
					color = tweak_data.screen_colors.text,
					blend_mode = "add",
...
function PrePlanningMapGui:update(t, dt)
	...
	if not self._is_pc_controller and (not (managers.system_menu and managers.system_menu:is_active()) or not not managers.system_menu:is_closing()) then
