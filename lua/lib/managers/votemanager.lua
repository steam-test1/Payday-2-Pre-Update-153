...
function VoteManager:_host_register(peer_id, response)
	if not (self._vote_response and self._vote_response[peer_id]) or self._vote_response[peer_id] ~= self.VOTES.none then
		return
	end
...
function VoteManager:message_vote()
	...
	if game_state_machine:current_state_name() == "menu_main" then
		if not (managers.menu:active_menu() and managers.menu:active_menu().logic:selected_node()) or managers.menu:active_menu().logic:selected_node():parameters().name ~= "lobby" then
			return
		end
	elseif game_state_machine:current_state_name() == "ingame_waiting_for_players" then
		if not (managers.menu:active_menu() and managers.menu:active_menu().logic:selected_node()) or managers.menu:active_menu().logic:selected_node():parameters().name ~= "kit" then
			return
		end
	elseif not (managers.menu:active_menu() and managers.menu:active_menu().logic:selected_node()) or managers.menu:active_menu().logic:selected_node():parameters().name ~= "pause" then
		return
	end
