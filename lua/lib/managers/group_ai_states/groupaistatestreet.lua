...
function GroupAIStateStreet:_upd_assault_task(task_data)
	...
	if 0 < nr_wanted and assault_data.phase ~= "fade" and not next(self._spawning_groups) then
...
function GroupAIStateStreet:_upd_reenforce_tasks()
	...
	while 0 < i do
		...
		elseif force_required and 0 < force_required then
	...
	elseif assault_data.phase == "sustain" then
		...
		if enemies_left < 7 or t > assault_data.phase_end_t + 350 then
			if t > assault_data.phase_end_t - 8 and not assault_data.said_retreat then
				...
				if t > assault_data.phase_end_t and self._drama_data.amount < tweak_data.drama.assault_fade_end and self:_count_criminals_engaged_force(4) <= 3 then
