...
function HuskPlayerMovement:anim_cbk_spawn_melee_item(unit, graphic_object)
	if not (not alive(self._melee_item_unit) and managers.network:session()) or not managers.network:session():peer_by_unit(self._unit) then
		return
	end
...
function HuskPlayerMovement:_update_position(t, dt)
	...
	if self._moving then
		...
		while 0 < remaining_move_speed do
...
function HuskPlayerMovement:sync_action_walk_nav_point(pos, action, params)
	...
	local can_add = true
	if not params.force and 0 < path_len then
