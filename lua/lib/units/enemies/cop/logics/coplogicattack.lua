		...
		if focus_enemy.verified or focus_enemy.nearly_visible then
			...
			if aim == nil and focus_enemy.reaction >= AIAttentionObject.REACT_AIM then
				if focus_enemy.reaction >= AIAttentionObject.REACT_SHOOT then
					...
					aim = aim or shoot
					if not aim and focus_enemy.verified_dis < (running and data.internal_data.weapon_range and (data.internal_data.weapon_range.close or data.internal_data.weapon_range.far) or 500) then
						aim = true
					end
				else
					aim = true
			...
			if not aim then
				...
				if expected_pos then
					if running then
						...
						if watch_pos_dis < 500 or watch_pos_dis < 1000 and 0.85 < watch_walk_dot then
							aim = true
						end
					else
						aim = true
...
function CopLogicAttack._chk_exit_non_walkable_area(data)
	local my_data = data.internal_data
	if not (not my_data.advancing and CopLogicAttack._can_move(data)) or data.unit:movement():chk_action_forbidden("walk") then
		return
	end
	local my_tracker = data.unit:movement():nav_tracker()
