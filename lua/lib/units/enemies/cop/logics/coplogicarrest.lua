...
function CopLogicArrest._upd_enemy_detection(data)
	...
	if not ((not new_attention or current_attention) and (not current_attention or new_attention)) or new_attention and current_attention.u_key ~= new_attention.u_key then
...
function CopLogicArrest._get_att_obj_close_pos(data, my_data)
	...
	if my_dis > optimal_dis * 0.8 and my_dis < optimal_dis * 1.2 and nav_manager:is_pos_free({
	...
	if not ray_res and nav_manager:is_pos_free({
	...
	if hit_dis_error < my_error and nav_manager:is_pos_free({
		position = hit_pos,
		radius = 40,
		filter = data.pos_rsrv_id
	}) then
