...
function TaserLogicAttack._chk_reaction_to_attention_object(data, attention_data, stationary)
	...
	if not (not (reaction < AIAttentionObject.REACT_SHOOT) and attention_data.criminal_record) or not attention_data.is_person then
		return reaction
	end
