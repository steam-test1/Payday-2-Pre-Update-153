	...
	for _, path in ipairs(all_paths) do
		for idx, current_point_data in ipairs(path.points) do
			...
			if point_in_front then
				...
				if distance <= closest_point.distance and 500 < distance then
...
function NpcVehicleStateManeuver:_is_point_in_front(target_position, cop_position)
	...
	local in_front = 0 < target_spin_relative_to_vehicle and fov > target_spin_relative_to_vehicle or target_spin_relative_to_vehicle > 360 - fov and target_spin_relative_to_vehicle < 360
	return in_front
end
