...
function Drill:_register_sabotage_SO()
	if not (not self._sabotage_SO_id and managers.navigation:is_data_ready() and self._unit:timer_gui() and self._unit:timer_gui()._can_jam) or not self._sabotage_align_obj_name then
		return
	end
	...
	local act_anim = "sabotage_device_" .. (100 < height and "high" or 60 < height and "mid" or "low")
...
function Drill:set_skill_upgrades(upgrades)
	...
	if self.is_drill or self.is_saw then
		...
		if 0 < auto_repair_level_1 or 0 < current_auto_repair_level_1 or 0 < auto_repair_level_2 or 0 < current_auto_repair_level_2 then
